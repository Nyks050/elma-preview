(()=>{
  ...
  function arrivalSummary(start,seconds){const safeSeconds=Math.max(0,Number(seconds)||0),end=new Date(start.getTime()+safeSeconds*1000),minutes=Math.max(1,Math.round(safeSeconds/60));return 'Tahmini varış '+fmtTime(end)+' • '+minutes+' dk.'}
  function styles(){
  ...
  function makeSheet(origin,destination,drive,walk,transit,preferred){
    styles();sheet?.remove();sheet=document.createElement('section');sheet.id='elmaJourneyResults';sheet.className='jr-sheet';
    const now=new Date(),transitRange=transit?arrivalSummary(now,transit.seconds):'',driveRange=drive?arrivalSummary(now,drive.seconds):'',walkRange=walk?arrivalSummary(now,walk.seconds):'';
