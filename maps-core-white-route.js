(()=>{
  ...
  function animateRoute(path,duration=6500){
    if(!path?.length)return Promise.resolve();
    if(routeAnimationFrame)cancelAnimationFrame(routeAnimationFrame);
    fallbackRouteLine?.setMap(null);
    routeBaseLine?.setMap(null);
    routeBaseLine=new google.maps.Polyline({
      map,path,strokeColor:'#050505',strokeWeight:5,strokeOpacity:.10,
      clickable:false,zIndex:3
    });
    fallbackRouteLine=new google.maps.Polyline({
      map,path:[path[0]],strokeColor:'#050505',strokeWeight:6,strokeOpacity:1,
  ...
  async function init(){
    ...
    map=new google.maps.Map(fresh,{
      ...
      styles:[
        ...
        {elementType:'labels.text.fill',stylers:[{color:'#575a5f'}]},
        {elementType:'labels.text.stroke',stylers:[{color:'#ffffff'},{weight:4}]},
        {featureType:'administrative',elementType:'geometry.stroke',stylers:[{color:'#d9dadd'}]},
        {featureType:'landscape',elementType:'geometry',stylers:[{color:'#ffffff'}]},
        {featureType:'poi',elementType:'geometry',stylers:[{color:'#f7f7f8'}]},
        {featureType:'poi.park',elementType:'geometry',stylers:[{color:'#f3f6f3'}]},
        {featureType:'road',elementType:'geometry',stylers:[{color:'#ececef'}]},
        {featureType:'road',elementType:'geometry.stroke',stylers:[{color:'#ffffff'}]},
        {featureType:'road.highway',elementType:'geometry',stylers:[{color:'#e1e2e5'}]},
        {featureType:'transit',elementType:'geometry',stylers:[{color:'#f1f1f3'}]},
        {featureType:'water',elementType:'geometry',stylers:[{color:'#e8f1f5'}]}
      ]
    });
