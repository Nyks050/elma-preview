(()=>{
  ...
  function liveInfo(line){
    ...
    const upcoming=schedule.times.filter(time=>{const [h,m]=time.split(':').map(Number);return h*60+m>=minute}).slice(0,3);
    const first=upcoming[0];
    const minutes=first?Math.max(0,first.split(':').map(Number).reduce((h,m)=>h*60+m)-minute):null;
    return{data,schedule,upcoming,minutes,active:Boolean(first)};
  }
  ...
  function routeStrip(line,info){
    ...
    const times=info.upcoming.length?info.upcoming.join(' · '):'Bugünkü seferler tamamlandı';
      ...
      <div class="el-lines-row">
        <button type="button" class="el-lines-summary" data-toggle-line="${line}" aria-expanded="${open}">
          ...
          <span class="el-lines-route">
            <span class="el-lines-name">${lineName(line)}</span>
          ...
          <span class="el-lines-live">
            <b>${info.data?minuteLabel(info.minutes):'—'}</b>
            <small class="${info.active?'active':''}"><i></i>${info.active?'Tarifede':'Bilgi yok'}</small>
          </span>
  ...
  async function mount(){
    ...
    root.innerHTML=`<div class="el-lines-toolbar">
      ...
      <div class="el-lines-filters" aria-label="Hat filtreleri">
        ...
        <button type="button" class="el-lines-filter" data-filter="active">Şimdi çalışan</button>
