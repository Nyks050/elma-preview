(()=>{
  ...
  function renderList(){
    ...
    clearPreviews();
    wrap.querySelector('.erm-cards').innerHTML=visible.length?visible.map(line=>'<article class="erm-card"><button class="erm-card-open" type="button" data-line="'+line.id+'" aria-label="'+title(line)+' güzergâhını haritada aç"><span class="erm-card-head"><b class="erm-card-number">'+numberHTML(line)+'</b></span><span class="erm-thumb" data-preview="'+line.id+'"></span><span class="erm-card-foot"><span><i></i> GÜZERGÂH</span><b>'+distance(line.route)+'</b></span></button><button class="erm-heart'+(favorites.has(line.id)?' active':'')+'" type="button" data-favorite="'+line.id+'" aria-label="'+(favorites.has(line.id)?'Favorilerden çıkar':'Favorilere ekle')+'"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.8-8.6a5.5 5.5 0 0 0 0-7.8Z"/></svg></button></article>').join(''):'<div class="erm-no-results">Hat bulunamadı.</div>';
  ...
  function styles(){
    ...
    css.textContent=`
      ...
      .erm-cards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.erm-card{position:relative;min-width:0;overflow:hidden;border:1px solid var(--border);border-radius:17px;background:var(--surface)}.erm-card-open{display:block;width:100%;padding:10px;border:0;background:transparent;color:var(--ink);text-align:left}.erm-card-head{display:flex;align-items:center;gap:9px;height:50px;padding:0 27px 0 10px;border-left:4px solid var(--ink)}.erm-card-number{display:flex;width:53px;height:43px;flex:none;flex-direction:column;align-items:center;justify-content:center;border-right:1px solid var(--border);color:var(--ink);font-size:27px;font-weight:850;line-height:1}.erm-card-number small{margin-top:1px;font-size:9px;letter-spacing:.4px}.erm-heart{position:absolute;top:20px;left:clamp(82px,calc(40% + 14px),108px);width:28px;height:28px;display:grid;place-items:center;padding:0;border:0;background:none;color:#747989}.erm-heart.active{color:var(--ink)}.erm-heart svg{width:21px;height:21px;fill:transparent;stroke:currentColor;stroke-width:1.7}.erm-heart.active svg{fill:currentColor}
      ...
      @media(max-width:370px){.erm{padding-right:10px;padding-left:10px}.erm-cards{gap:7px}.erm-card-open{padding:8px}.erm-card-head{gap:5px;padding-left:7px}.erm-card-number{width:39px;height:37px;font-size:23px}.erm-card-number small{font-size:8px}.erm-thumb{height:85px}.erm-card-foot{font-size:8px}.erm-card-foot b{font-size:10px}}
  ...
  function mount(){
    ...
    wrap.querySelector('.erm-cards').onclick=event=>{
      const favorite=event.target.closest('[data-favorite]');if(favorite){const id=favorite.dataset.favorite;favorites.has(id)?favorites.delete(id):favorites.add(id);try{localStorage.setItem(FAVORITES_KEY,JSON.stringify([...favorites]))}catch{};window.dispatchEvent(new Event('elma-route-favorites-changed'));return}
