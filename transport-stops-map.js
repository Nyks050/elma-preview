(()=>{
  ...
  function renderList(){
    ...
    clearPreviews();
    wrap.querySelector('.est-cards').innerHTML=matches.length?matches.map(line=>'<button class="est-card" type="button" data-line="'+line.id+'" aria-label="'+title(line)+' duraklarını haritada aç"><span class="est-card-head"><b class="est-number">'+number(line)+'</b><strong>'+title(line)+'</strong><span class="est-arrow">›</span></span><span class="est-thumb" data-preview="'+line.id+'"></span><span class="est-card-foot"><span><i></i> DURAKLAR</span><b>'+line.stops.length+' durak</b></span></button>').join(''):'<div class="est-empty">Hat veya durak bulunamadı.</div>';
  ...
  function styles(){
    ...
    css.textContent=`
      ...
      @media(max-width:370px){.est{padding-right:10px;padding-left:10px}.est-cards{gap:7px}.est-card{padding:8px}.est-card-head{gap:7px}.est-number{min-width:37px;height:37px;font-size:14px}.est-card-head strong{font-size:11px}.est-thumb{height:85px}.est-card-foot{font-size:8px}.est-card-foot b{font-size:10px}}
