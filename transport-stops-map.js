(()=>{
  ...
  let lines=[],selected='',map,libPromise,previewObserver;
  ...
  function renderList(){
    ...
    clearPreviews();
    wrap.querySelector('.est-cards').innerHTML=matches.length?matches.map(line=>'<button class="est-card" type="button" data-line="'+line.id+'" aria-label="'+title(line)+' duraklarını haritada aç"><span class="est-card-head"><b class="est-number">'+numberHTML(line)+'</b><strong>'+title(line)+'</strong><span class="est-arrow">›</span></span><span class="est-thumb" data-preview="'+line.id+'"></span><span class="est-card-foot"><span><i></i> DURAKLAR</span><b>'+line.stops.length+' durak</b></span></button>').join(''):'<div class="est-empty">Hat veya durak bulunamadı.</div>';
  ...
  async function select(id){
    ...
    wrap.querySelector('.est-detail-title').textContent=title(line);
  ...
  function styles(){
    ...
    css.textContent=`
      ...
      .est-cards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.est-card{display:block;min-width:0;padding:10px;overflow:hidden;border:1px solid var(--border);border-radius:17px;background:var(--surface);color:var(--ink);text-align:left}.est-card-head{display:flex;align-items:center;gap:9px;height:50px;padding-left:10px;border-left:4px solid var(--ink)}.est-number{display:flex;width:53px;height:43px;flex:none;flex-direction:column;align-items:center;justify-content:center;border-right:1px solid var(--border);color:var(--ink);font-size:27px;font-weight:850;line-height:1}.est-number small{margin-top:1px;font-size:9px;letter-spacing:.4px}.est-card-head strong{min-width:0;flex:1;overflow:hidden;font-size:12px;font-weight:800;text-overflow:ellipsis;white-space:nowrap}.est-arrow{color:var(--muted);font-size:20px}.est-thumb{display:block;height:98px;margin:9px 0 10px;overflow:hidden;border-radius:10px;background:#eef0ec;pointer-events:none}.est-card-foot{display:flex;justify-content:space-between;gap:4px;color:var(--muted);font-size:9px;font-weight:750}.est-card-foot span{white-space:nowrap}.est-card-foot i{display:inline-block;width:6px;height:6px;margin-right:5px;border-radius:50%;background:var(--ink);vertical-align:1px}.est-card-foot b{color:var(--ink);font-size:11px;white-space:nowrap}.est-empty{grid-column:1/-1;padding:35px;color:var(--muted);text-align:center}
      ...
      @media(max-width:370px){.est{padding-right:10px;padding-left:10px}.est-cards{gap:7px}.est-card{padding:8px}.est-card-head{gap:5px;padding-left:7px}.est-number{width:39px;height:37px;font-size:23px}.est-number small{font-size:8px}.est-card-head strong{font-size:11px}.est-thumb{height:85px}.est-card-foot{font-size:8px}.est-card-foot b{font-size:10px}}
