(()=>{
  ...
  let lines=[],selected='',map,libPromise,previewObserver,favorites=new Set();
  ...
  function library(){
    ...
    libPromise=new Promise((resolve,reject)=>{
      if(!document.getElementById('estMapCSS')){const css=document.createElement('link');css.id='estMapCSS';css.rel='stylesheet';css.href='https://unpkg.com/maplibre-gl@5.12.0/dist/maplibre-gl.css';document.head.appendChild(css)}
  ...
  async function preview(element,line){
    ...
    try{
      ...
      previews.set(line.id,instance);instance.on('load',()=>draw(instance,line,false));
    }catch(error){console.warn('Durak önizlemesi:',error)}
  }
  function clearPreviews(){previewObserver?.disconnect();for(const instance of previews.values())instance.remove();previews.clear()}
  function renderList(){
    ...
    clearPreviews();
    wrap.querySelector('.est-cards').innerHTML=matches.length?matches.map(line=>'<article class="est-card"><button class="est-card-open" type="button" data-line="'+line.id+'" aria-label="'+title(line)+' duraklarını haritada aç"><span class="est-card-head"><b class="est-number">'+numberHTML(line)+'</b></span><span class="est-thumb" data-preview="'+line.id+'"></span><span class="est-card-foot"><span><i></i> DURAKLAR</span><b>'+line.stops.length+' durak</b></span></button><button class="est-heart'+(favorites.has(line.id)?' active':'')+'" type="button" data-favorite="'+line.id+'" aria-label="'+(favorites.has(line.id)?'Favorilerden çıkar':'Favorilere ekle')+'"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.8-8.6a5.5 5.5 0 0 0 0-7.8Z"/></svg></button></article>').join(''):'<div class="est-empty">Hat veya durak bulunamadı.</div>';
  ...
  function styles(){
    ...
    css.textContent=`
      ...
      .est-cards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.est-card{position:relative;min-width:0;overflow:hidden;border:1px solid var(--border);border-radius:17px;background:var(--surface)}.est-card-open{display:block;width:100%;padding:10px;border:0;background:transparent;color:var(--ink);text-align:left}.est-card-head{display:flex;align-items:center;gap:9px;height:50px;padding-left:10px;border-left:4px solid var(--ink)}.est-number{display:flex;width:53px;height:43px;flex:none;flex-direction:column;align-items:center;justify-content:center;border-right:1px solid var(--border);color:var(--ink);font-size:27px;font-weight:850;line-height:1}.est-number small{margin-top:1px;font-size:9px;letter-spacing:.4px}.est-heart{position:absolute;top:20px;left:clamp(82px,calc(40% + 14px),108px);width:28px;height:28px;display:grid;place-items:center;padding:0;border:0;background:none;color:var(--muted)}.est-heart.active{color:var(--ink)}.est-heart svg{width:21px;height:21px;fill:transparent;stroke:currentColor;stroke-width:1.7}.est-heart.active svg{fill:currentColor}.est-thumb{display:block;height:98px;margin:9px 0 10px;overflow:hidden;border-radius:10px;background:#eef0ec;pointer-events:none}.est-card-foot{display:flex;justify-content:space-between;gap:4px;color:var(--muted);font-size:9px;font-weight:750}.est-card-foot span{white-space:nowrap}.est-card-foot i{display:inline-block;width:6px;height:6px;margin-right:5px;border-radius:50%;background:var(--ink);vertical-align:1px}.est-card-foot b{color:var(--ink);font-size:11px;white-space:nowrap}.est-empty{grid-column:1/-1;padding:35px;color:var(--muted);text-align:center}
      .est-back{display:block;margin:0 0 15px;padding:4px 0;border:0;background:none;color:var(--muted);font-size:13px!important;font-weight:750!important}.est-detail-head{display:flex;align-items:center;gap:10px;margin-bottom:12px}.est-detail-number{display:grid;min-width:43px;height:43px;place-items:center;padding:0 6px;border-radius:11px;background:#171719;color:#fff;font-size:16px;font-weight:850}.est-detail-title{min-width:0;flex:1;font-size:15px;font-weight:800}.est-fit{width:38px;height:38px;border:1px solid var(--border);border-radius:10px;background:var(--surface);color:var(--ink);font-size:23px!important}.est-map{height:min(62dvh,550px);min-height:310px;overflow:hidden;border:1px solid var(--border);border-radius:15px;background:#e9e9e9}.est-map .maplibregl-ctrl-attrib{font-size:10px}.est-count{display:block;margin-top:13px;color:var(--ink);font-size:13px;font-weight:800;text-align:right}
      @media(max-width:370px){.est{padding-right:10px;padding-left:10px}.est-cards{gap:7px}.est-card-open{padding:8px}.est-card-head{gap:5px;padding-left:7px}.est-number{width:39px;height:37px;font-size:23px}.est-number small{font-size:8px}.est-thumb{height:85px}.est-card-foot{font-size:8px}.est-card-foot b{font-size:10px}}
  ...
  function mount(){
    ...
    wrap.innerHTML='<div class="est-list"><label class="est-search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5"/><path d="m16 16 5 5"/></svg><input type="search" aria-label="Hat veya durak ara" placeholder="Hat veya durak ara"></label><div class="est-cards"><div class="est-empty">Duraklar yükleniyor…</div></div></div><div class="est-detail" hidden><button class="est-back" type="button">‹ Duraklar</button><div class="est-detail-head"><b class="est-detail-number"></b><strong class="est-detail-title"></strong><button class="est-fit" type="button" aria-label="Tüm durakları göster">⌖</button></div><div class="est-map" aria-label="OpenStreetMap durak haritası"></div><b class="est-count"></b></div>';
    target.appendChild(wrap);
    wrap.querySelector('input').oninput=renderList;
    wrap.querySelector('.est-cards').onclick=event=>{const favorite=event.target.closest('[data-favorite]');if(favorite){const id=favorite.dataset.favorite;favorites.has(id)?favorites.delete(id):favorites.add(id);try{localStorage.setItem(FAVORITES_KEY,JSON.stringify([...favorites]))}catch{};window.dispatchEvent(new Event('elma-route-favorites-changed'));return}const button=event.target.closest('[data-line]');if(button)select(button.dataset.line)};
    window.addEventListener('elma-route-favorites-changed',renderList);
    ...
    new MutationObserver(()=>{if(target.classList.contains('active')&&map&&current())fit(map,current().route,38)}).observe(target,{attributes:true,attributeFilter:['class']});
    fetch(DATA_URL).then(response=>{if(!response.ok)throw Error(response.status);return response.json()}).then(data=>{lines=data.lines.filter(line=>line.route?.length&&line.stops?.length);renderList()}).catch(()=>{wrap.querySelector('.est-cards').innerHTML='<div class="est-empty">Duraklar yüklenemedi. Tekrar deneyin.</div>'});
  }
