...
Bu değişiklik **Firebase sunucu dağıtımı tamamlanmadan yayımlanmaz**. Gizlilik Politikası metni `2026-09-24-v1` sürümüyle yerleştirildi. Bu sürümün kurallarla eşleşmemesi mevcut kullanıcıların yeni kayıt ve ilan oluşturmasını engeller.

1. Metnin `index.html`, `app-guncel.html` ve `privacy.html` içinde birebir aynı olduğundan; `window.ELMAGO_POLICY_VERSION`, `firestore.rules` ve `storage.rules` içinde sürümün `2026-09-24-v1` olduğundan emin ol.
