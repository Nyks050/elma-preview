(()=>{
  ...
  async function initMap(container){
    ...
    routeMarkers=stops.map((position,index)=>{const isReturn=index>=25,number=isReturn?index-24:index+1,color=isReturn?'#0b8f9c':'#050506';routeBounds.extend(point(position));return new google.maps.Marker({map:routeMap,position:point(position),zIndex:100+index,title:`6 Nolu Hat • ${isReturn?'Dönüş':'Gidiş'} • Durak ${number}`,label:{text:isReturn?'D'+number:String(number),color,fontSize:'8px',fontWeight:'800'},icon:{path:google.maps.SymbolPath.CIRCLE,scale:9,fillColor:'#fff',fillOpacity:1,strokeColor:color,strokeOpacity:1,strokeWeight:2}})});
    const outboundPath=window.ELMA_FIXED_ROAD_PATHS?.line6Outbound||[],returnPath=window.ELMA_FIXED_ROAD_PATHS?.line6Return||[];
    if(outboundPath.length&&returnPath.length){[...outboundPath,...returnPath].forEach(position=>routeBounds.extend(point(position)));routeLines=[new google.maps.Polyline({map:routeMap,path:outboundPath.map(point),strokeColor:'#050506',strokeOpacity:.96,strokeWeight:6,geodesic:false,zIndex:5}),new google.maps.Polyline({map:routeMap,path:returnPath.map(point),strokeColor:'#0b8f9c',strokeOpacity:.96,strokeWeight:6,geodesic:false,zIndex:6})]}
