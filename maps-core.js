(()=>{
  ...
  function dragSheet(){
    ...
    grab.onpointerdown=event=>{dragging=true;startY=event.clientY;grab.setPointerCapture?.(event.pointerId)};
    ...
    grab.onpointerup=()=>{dragging=false;if(deltaY>90){sheet.classList.add('dismissed');restore.classList.add('show')}sheet.style.transform='';deltaY=0};
    restore.onclick=()=>{restore.classList.remove('show');sheet.classList.remove('dismissed')};
  }
  ...
  async function init(){
    ...
    map=L.map('map',{zoomControl:false,attributionControl:false,preferCanvas:true}).setView(DEFAULT_CENTER,14);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png',{subdomains:'abcd',maxZoom:20}).addTo(map);
    map.createPane('elmaLabels');
    const labelPane=map.getPane('elmaLabels');
    labelPane.style.zIndex='350';
    labelPane.style.pointerEvents='none';
    labelPane.classList.add('elma-label-pane');
    L.tileLayer('https://{s}.basemaps.cartocdn.com/light_only_labels/{z}/{x}/{y}{r}.png',{subdomains:'abcd',maxZoom:20,pane:'elmaLabels'}).addTo(map);
...
(()=>{
  ...
  widgets.src='home-widgets.js?v=20260830-leaflet-fullscreen-labels';
  widgets.defer=true;
